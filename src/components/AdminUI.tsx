  ...
  return (
    ...
    >
      {loading ? (
        ...
        <>
          {icon}
          <Text style={[s.buttonText, { color: colors.fg }]} numberOfLines={1}>
            {title}
          </Text>
        </>
      )}
  ...
  return (
    <View style={containerStyle}>
      ...
      <View
        style={[
          s.inputFrame,
          focused && s.inputFrameFocused,
          error ? s.inputFrameError : null,
        ]}
      >
...
export const StatusBadge = React.memo(
  ({ label, tone = "neutral" }: { label: string; tone?: BadgeTone }) => {
    const s = useAdminUIStyles();
    ...
    return (
      ...
      >
        <Text style={[s.badgeText, { color: colors.fg }]} numberOfLines={1}>
          {label}
        </Text>
      </View>
    );
...
export const ReasonModal = ({
  ...
  const handleConfirm = () => {
    const finalReason =
      selectedPreset === "Other" ? reason.trim() : selectedPreset;
    if (!finalReason) {
      setError(true);
      return;
    }
  ...
  return React.useMemo(
    () =>
      StyleSheet.create({
        ...
        inputFrameFocused: { borderColor: colors.accent, backgroundColor: colors.surfaceElevated },
        inputFrameError: {
